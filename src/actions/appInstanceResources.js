...
const getAppInstanceResources = async ({
  ...
  try {
    ...
    let resources = appInstanceResources

    // use only given app instance resource id if defined
    if (appInstanceResourceId) {
      resources = [
        appInstanceResources.find(({ _id }) => _id === appInstanceResourceId)
      ]
    }
    return dispatch({
